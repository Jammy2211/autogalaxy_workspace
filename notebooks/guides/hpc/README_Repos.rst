...
To set up your PyAutoGalaxy workspace on an HPC system, the following commands
...
This environment will contain PyAutoGalaxy and all of its dependencies.
...
4. Create the PyAutoGalaxy Workspace
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

::

   mkdir autogalaxy_workspace
   cd autogalaxy_workspace

This creates and enters an ``autogalaxy_workspace`` directory on the HPC, which
will mirror your local PyAutoGalaxy workspace (excluding large datasets and
outputs).
