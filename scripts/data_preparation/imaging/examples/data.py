...
This image conforms to **PyAutoGalaxy** standards for the following reasons.
 ...
 - Units: The image flux is in units of electrons per second (as opposed to electrons, counts, ADU`s etc.). 
   Internal **PyAutoGalaxy** functions for computing quantities like a galaxy magnitude assume the data and model
...
Although **PyAutoGalaxy** can technically perform an analysis using other units, the default setup assumes electrons per 
...
"""
data_in_adus = ag.preprocess.array_eps_to_adus(
...
"""
data_large_stamp_trimmed = ag.preprocess.array_with_new_shape(
...
light from the sky, zodiacal light, and light from other galaxies in the field of view. The background should have
been subtracted from the image before it was reduced, but sometimes this is not the case.

It is recommend you use data processing tools outside of **PyAutoGalaxy** to subtract the background from your image,
as these have been optimized for this task. However, if you do not have access to these tools, **PyAutoGalaxy** has
...
# imaging_offset_centre = ag.Imaging.from_fits(data_path=path+`image.fits`, pixel_scales=0.1,
...
# imaging_recentred_pixels = ag.Imaging.from_fits(data_path=path+`image.fits`, pixel_scales=0.1,
#                                             noise_map_path=path+`noise_map.fits`,
#                                             psf_path=path+`psf.fits`,
