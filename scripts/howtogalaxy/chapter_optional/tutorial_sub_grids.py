...
This tutorial describes sub-grids. It is considered optional as the default sub-grid options of **PyAutoGalaxy** are
...
**PyAutoGalaxy** also provides a `OverSamplingIterate` input to a grid, which uses iteratively higher and higher 
levels of sub gridding to evaluate a `from_grid_` method until a threshold level of accuracy is met. This is used in 
many of the example `simulator` scripts to ensure that the images of simulated galaxies are evaluated accurately. 

This grid is not used by default in the majority of example scripts and a `sub_size=1` is assumed to ensure faster 
**PyAutoGalaxy** run times. 
"""
