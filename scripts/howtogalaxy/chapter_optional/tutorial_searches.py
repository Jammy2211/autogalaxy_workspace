...
By default **PyAutoGalaxy** use's Dynesty's Random Walk nested sampling (other approaches are available in Dynesty 
however our testing has revealed that the random walk sampling is best for modeling). In brief, in order for 
...
Through testing, we have found that `walks` = 5 - 10 is optimal for modeling. Higher values
...
**PyAutoGalaxy** performs best for `facc` = 0.2-0.3, but feel free to experiment with these values.
