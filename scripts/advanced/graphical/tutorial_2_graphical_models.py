...
In the previous tutorial we fitted individual galaxy models to individual galaxy datasets. We knew that every galaxy in
the dataset had same value of `sersic_index`, which we estimated using the weighted average of the Sersic indexes
inferred for each galaxy fit.

Graphical modeling follows a different approach. It composes a single model that is fitted to the entire galaxy dataset.
This model includes specific galaxy model component for every individual galaxy in the sample. However, the graphical
model also has shared parameters between these individual galaxy models.

This example fits a graphical model using the same sample fitted in the previous tutorial, consisting of
imaging data of three galaxies. We fit the `Sersic` model to each galaxy. However, whereas previously
the `sersic_index` of each galaxy model component was a free parameter in each fit, in the graphical model there is only
a single value of `sersic_index` shared by all three galaxies (which is how the galaxy data was simulated).
...
example, there are 3 galaxies each with their own galaxy model, therefore:
 ...
 - Each galaxy has 4 free parameters from the components of its `Sersic` that are not shared.

 - There are three galaxies in total, giving [3 x 4 = 12 free parameters]

 - There is one additional free parameter, which is the `sersic_index` shared by all 3 galaxies.

The overall dimensionality of parameter space is therefore N=13.
...
model is used.
"""
sersic_index_shared_prior = af.UniformPrior(lower_limit=0.8, upper_limit=5.0)
...
example, there are 3 galaxies each with their own galaxy model, therefore:
 ...
 - Each galaxy's bulge is a linear parametric `Sersic` bulge with its centre fixed to the input 
...
Above, we composed a `model_list` consisting of three galaxy models which each had a shared `sersic_index` prior. We 
also loaded three datasets which we intend to fit with each of these galaxy models, setting up each in an `Analysis` 
class that defines how the model is used to fit the data.

We now simply pair each galaxy model to each `Analysis` class, so that **PyAutoGalaxy** knows that: 
...
different model components that make up our model (e.g. the three `Collection` objects containig our `Galaxy`) and 
how their parameters are  linked or shared (e.g. that each `Galaxy` light profile has its own unique parameters, but 
a shared `sersic_index` parameter).
...
"""
We can now inspect the inferred value of `sersic_index`, and compare this to the value we estimated in the previous tutorial
...
"""
print(
    f"Weighted Average Sersic Index Estimate = 4.058408001873032 (0.29755239499365804) [1.0 sigma confidence intervals] \n"
)

sersic_index = result.samples.median_pdf()[0].galaxies.galaxy.bulge.sersic_index

u1_error = result.samples.values_at_upper_sigma(sigma=1.0)[
    0
].galaxies.galaxy.bulge.sersic_index
l1_error = result.samples.values_at_lower_sigma(sigma=1.0)[
    0
].galaxies.galaxy.bulge.sersic_index

u3_error = result.samples.values_at_upper_sigma(sigma=3.0)[
    0
].galaxies.galaxy.bulge.sersic_index
l3_error = result.samples.values_at_lower_sigma(sigma=3.0)[
    0
].galaxies.galaxy.bulge.sersic_index

print("Inferred value of the shared sersic_index via a graphical model fit: \n")
print(f"{sersic_index} ({l1_error} {u1_error}) [1.0 sigma confidence intervals]")
print(f"{sersic_index} ({l3_error} {u3_error}) [3.0 sigma confidence intervals]")

"""
__Wrap Up__

The graphical model's sersic_index estimate and errors are pretty much exactly the same as the weighted average!
...
same estimate of the galaxy sersic_index's as simpler approaches followed in the previous tutorial. 
