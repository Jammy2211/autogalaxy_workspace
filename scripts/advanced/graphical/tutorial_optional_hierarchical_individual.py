...
and the hierarchical parameters are fitted for simultaneously with the model parameters of each galaxy in each
dataset.

This script illustrates how the hierarchical parameters can be estimated using a simpler approach, which fits
each dataset one-by-one and estimates the hierarchical parameters afterwards by fitting the inferred `sersic_index`'s
...
for dataset_index, analysis in enumerate(analysis_list):
    ...
    nautilus = af.Nautilus(
        name="",
        path_prefix=Path("tutorial_optional_hierarchical_individual"),
...
print(f"Median PDF inferred sersic_index values")
print(mp_sersic_indexs)
print()
...
error_list = [ue1 - le1 for ue1, le1 in zip(ue1_sersic_indexs, le1_sersic_indexs)]

"""
The `Analysis` class below fits a Gaussian distribution to the inferred `sersic_index` values from each of the fits 
above, where the inferred error values are used as the errors.
"""
...
class Analysis(af.Analysis):
    ...
    def log_likelihood_function(self, instance: af.ModelInstance) -> float:
        ...
        In this example, this function determines from what parent Gaussian disrtribution the inferred Sersic index
        of each galaxy were drawn.
        """
...
"""
The `ParentGaussian` class is the model-component which used to fit the parent Gaussian to the 
inferred `sersic_index` values.
"""
