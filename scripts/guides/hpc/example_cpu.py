...
Below, we set `hpc_dataset_path=/hpc/data/hpc_username/dataset/example/simple__no_galaxy_light`.
"""
dataset_folder = "example"
dataset_name = "simple__no_galaxy_light"
...
__Batch Script Many Galaxyes__

HPC submissions require a batch script, which tells the HPC the CPU hardware you want the job to run on and the 
PyAutoGalaxy Python script you want it to execute. This script then distributes the job to nodes and CPUs. 

Lets look at the batch script 

 `autogalaxy_workspace/*/guides/hpc/batch/example_cpu_one_dataset_parallel
...
The following line activates the PyAutoGalaxy virtual environment we set up on hpc for this run:
...
Load and plot the strong galaxy dataset `example_image_1` via .fits files, which we will fit with the galaxy model.
"""
dataset_folder = "example"
dataset_name = "simple"
...
Define a 3.0" circular mask, which includes the emission of the galaxy and source galaxies.
"""
mask_radius = 3.0
...
galaxy = af.Model(ag.Galaxy, redshift=0.5, mass=mass, shear=shear)

# Source:

source = af.Model(ag.Galaxy, redshift=1.0, bulge=ag.lp.SersicCore)

# Overall Galaxy Model:
