"""
Results: Aggregator Start Here
==============================

This script is the starting point for analysing the results of modeling using the **aggregator**, which scrapes
a directory of completed fits and loads them on-demand via Python generators. This approach minimises memory usage
and is the right tool when you need to inspect many fits together (e.g. a sample of hundreds of galaxies).

If you only want to inspect a single fit quickly, the simpler alternative is to load files directly from the
`output/` folder. See `autogalaxy_workspace/*/guides/results/start_here.py` for that workflow — it loads
`.json` and `.fits` files instantly into full Python objects, at the cost of holding everything in memory.

  - Simple loading (fast, in-memory)         : `guides/results/start_here.py`
  - Aggregator (scraped, generator-based)    : this file

After reading this script, the sibling files in `aggregator/` provide more detailed examples for analysing different
aspects of the results outlined here.
...
The examples script `autogalaxy_workspace/*/results/aggregator/samples.py` provides a detailed description of 
