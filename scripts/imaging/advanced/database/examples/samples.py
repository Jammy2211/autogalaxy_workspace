...
In the script `autogalaxy_workspace/*/advanced/database/start_here.py` we performed a fit which fitted 3
...
"""
instance_list = [samps.max_log_likelihood() for samps in agg.values("samples")]
print("Maximum Log Likelihood Model Instances: \n")
print(instance_list, "\n")

"""
The attributes of the `instance` (e.g. `galaxies`, `galaxy`) have these names due to how we composed the `Galaxy` and
its light and mass profiles via the `Collection` and `Model` above. 
"""
print(instance_list[0].galaxies)
print(instance_list[1].galaxies)
print(instance_list[2].galaxies)

"""
These galaxies will be named according to the model composed and fitted by the search (in this case, only `galaxy`).
"""
print(instance_list[0].galaxies.galaxy)
print()
print(instance_list[1].galaxies.galaxy)

"""
Their light profiles are also named according to model composition allowing individual parameters to be printed.
"""
print(instance_list[0].galaxies.galaxy.bulge.effective_radius)
print(instance_list[1].galaxies.galaxy.bulge.effective_radius)
...
This fit used `nautilus` therefore we use the `DynestyPlotter` for visualization, which wraps `nautilus`'s in-built
visualization tools.
...
print(
    "All parameters of the very first sample (containing only the effective radius and sersic index)."
)
print(samples.parameter_lists[0])

print(
    "Maximum Log Likelihood Model Instances (containing only the effective radius and sersic index):\n"
)
...
with the API used to compose a model.
...
print(
    "All parameters of the very first sample (containing only the galaxy mass's einstein radius and "
    "source bulge's sersic index)."
)

"""
Above, we filtered the `Samples` but asking for all parameters which included the
path ("galaxies", "lens", "mass", "effective_radius").
...
samples = samples.without_paths(["galaxies.galaxy.bulge.centre.centre_0"])

print("Parameters of first sample without the galaxy mass centre.")
print(samples.parameter_lists[0])

"""
We can keep and remove entire paths of the samples, for example keeping only the parameters of the galaxy or 
