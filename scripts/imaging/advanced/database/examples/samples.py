...
After fitting a large suite of galaxy data, we can use the aggregator to load the database's results. We can then
manipulate, interpret and visualize them using a Python script or Jupyter notebook.

This script uses the results generated by the script `/autogalaxy_workspace/database/tutorial_1_introduction.py`, which
fitted 3 simulated galaxies where:

 - The galaxy's light is a parametric `Sersic` bulge and `Exponential` disk.

__Samples__

This script covers how to manipulate the `Samples` object returned from a *PyAutoGalaxy* model-fit, which you have most
...
"""
These galaxies will be named according to the search (in this case, `galaxy`).
"""
...
"""
print(instance_list[0].galaxies.galaxy.bulge)
print(instance_list[1].galaxies.galaxy.bulge)
...
The following lists of parameter quantities are available as a property of the `Model`, the order of which all 
correspond to the parameters in the `max_lh_vector` above:
