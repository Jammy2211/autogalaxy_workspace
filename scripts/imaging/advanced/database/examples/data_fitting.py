...
In tutorial  6, we will show how one can use the database to create lists of `Plane`'s that are drawn from the PDF.
...
"""
The masks we used to fit the galaxyes is accessible via the aggregator.
"""
...
compose our own `Imaging` object. For large datasets, this would require us to use generators to ensure it is memory-light,
...
customize the plots using the PyAutoGalaxy mat_plot_2d.
