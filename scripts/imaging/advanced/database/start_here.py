...
have a need to perform many model fits to large samples of galaxies, making manual inspection of results time consuming.

PyAutoGalaxy's database feature outputs all model-fitting results as a
...
for dataset_name in dataset_names:
    ...
    settings_dataset = ag.SettingsImaging(grid_class=ag.Grid2D, sub_size=1)
    ...
    contains the (hypothetical) redshift and velocity dispersion of every galaxy in our sample.
    """
    ...
    lens datasets have a `true_plane.pickle` file which we pass in below, which we use in the `Aggregator` tutorials 
    to check if the model-fit recovers its true input parameters.
    """
    pickle_files = [path.join(dataset_path, "true_plane.pickle")]
...
"""
If you inspect the `autogalaxy_workspace/output/database` folder during the model-fit, you'll see that the results
