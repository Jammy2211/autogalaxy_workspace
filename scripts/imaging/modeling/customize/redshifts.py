...
The redshift of the galaxy  galaxies are also input.

All `LightProfile`'s and `MassProfiles`'s in  **PyAutoGalaxy** are defined in units that mean the redshifts do not
change the modeling results. For example, because the *einstein_radius_ parameter of the `Isothermal` is
in units arc-seconds, changing the galaxy  redshifts does not change the model behaviour.
