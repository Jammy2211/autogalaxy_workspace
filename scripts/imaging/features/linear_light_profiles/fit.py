...
print(
    f"\n Intensity of galaxy's bulge = {fit.linear_light_profile_intensity_dict[bulge]}"
)

print(
    f"\n Intensity of galaxy's disk  = {fit.linear_light_profile_intensity_dict[disk]}"
)
...
For example, the linear light profile `Sersic` of the `bulge` component above has a solved for `intensity` of ~1.0.. 

The `Galaxies` created below instead has an ordinary light profile with an `intensity` of ~1.0.
