...
shapelet_0 = ag.lp.ShapeletPolar(
    n=0,
    m=0,
    centre=(0.0, 0.0),
    ell_comps=(0.0, 0.0),
...
for i in range(total_n + total_m):
    shapelet = ag.lp.ShapeletPolarSph(
...
These coefficients are visualized below using `aplt.subplot_basis_image`.
"""
...
shapelet_0 = ag.lp_linear.ShapeletPolar(
    n=0,
    m=0,
    centre=(0.0, 0.0),
    ell_comps=(0.0, 0.0),
...
"""
We can use `aplt.subplot_basis_image` to plot each individual shapelet in the reconstructed basis.
...
"""
bulge = fit.galaxies[0].bulge

print(
    f"\n Intensity of source galaxy's first shapelet in bulge = {fit.linear_light_profile_intensity_dict[bulge.profile_list[0]]}"
)
...
As a rule of thumb, modeling is generally better if a pixelization is used to reconstruct the galaxy's light,
