...
There may be regions of an image that have signal near the galaxy  that is from other sources (e.g. foreground
stars, background galaxies not associated with the galaxy). The emission from these images will impact our
...
"""
Now we`re happy with the image, lets output it to the dataset folder of the galaxy, so that we can load it from a .fits
...
`autogalaxy_workspace/*/data_preparation/imaging/gui/scaled_dataset.py`. This tools allows you `spray paint` on the image where 
