...
Setup the path to the autogalaxy_workspace, using the correct path name below.

The `dataset label` is the name of the folder in the `autogalaxy_workspace/dataset` folder and `dataset_name` the 
folder the dataset is stored in, e.g, `/autogalaxy_workspace/dataset/dataset_type/dataset_name`. The mask will be 
...
"""
First, load the image of the dataset, so that the mask can be plotted over the galaxy.
"""
...
# mask = ag.Mask2D.elliptical_annular(
#     shape_native=data.shape_native,
#     pixel_scales=data.pixel_scales,
