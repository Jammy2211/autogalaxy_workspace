...
This image conforms to **PyAutoGalaxy** standards for the following reasons.
 ...
 - Stamp Size: The image is a postage stamp cut-out of the lens, but does not include many pixels around the edge of
   ...
   the amount of memory **PyAutoGalaxy** uses, speeds up the analysis and ensures visualization zooms around the lens
   and source. However, conforming to this standard is not necessary to ensure an accurate **PyAutoGalaxy** analysis.
...
The mask removes the regions of the image where galaxy is not present, typically the edges of the 
image.
...
This script shows how to create customize masked (e.g. annular, ellipses) which are tailored to match the galaxy structures.
...
you are analysing. These can be used as fixed values for the light models centre in a model-fit.

This reduces the number of free parameters fitted for in a model and removes inaccurate solutions where
the model centre is unrealistically far from its true centre.
...
centre to help the non-linear search find a good model.
...
There may be galaxies nearby the galaxy you are fitting, whose emission blends with the galaxy you are studying,

We can include these galaxies in the model as light profiles, using the
...
are included in the model the centre of these extra galaxies light are fixed to these 
...
with the galaxy we are studying. The emission from these images will impact our model fitting and needs to be 
...
Auxiliary information about a galaxy dataset may used during an analysis or afterwards when interpreting the 
 modeling results. For example, the redshifts of the galaxy. 
...
the results of a model fit additional data on a galaxy can be used to. 

For example, to plot the model-results against other measurements of a galaxy not made by PyAutoGalaxy. Examples of such 
data might be:

- The velocity dispersion of the galaxy.
- The stellar mass of the galaxy.
- The results of previous galaxy models to the performed in previous papers.
