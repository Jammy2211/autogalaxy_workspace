...
This tool allows one to mask a bespoke noise-map for a given image of a galaxy, using a GUI.

This noise-map is primarily used for increasing the variances of pixels that have non-modeled components in an image,
for example intervening line-of-sight galaxies that are near the galaxy, but not directly interfering with the
analysis of the galaxy  galaxies.
...
"""
scribbler = ag.Scribbler(image=data.native, cmap=cmap)
...
"""
background_level = ag.preprocess.background_noise_map_via_edges_from(
...
data = ag.Array2D.no_mask(values=data, pixel_scales=pixel_scales)
...
"""
Output image to the dataset folder of the galaxy, so that we can load it from a .fits file for modeling.
"""
data.output_to_fits(
...
noise_map = ag.Array2D.no_mask(values=noise_map, pixel_scales=pixel_scales)
